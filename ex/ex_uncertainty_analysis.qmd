...
Unfortunately, this result is not enough to reach a conclusion whether a High Exposure is below the Reference Point (i.e. that the MOE is greater than `r th`). See Post scriptum 3 for how the refined option with probability bounds can be designed to cosnider what level of certainty that is required. 
...
## Extra material outside of the exercise
### Post scriptum 1
...
6) Elicit a conservative estimate of the Reference Point from the toxicology experts (asking them to consider the identified sources of uncertainty for the hazard assessment).

E.g. $P(\frac{RP}{UF} < y) = 85\%$
