...
A Reference Point of `r bmdl` μg iAs/kg bw per day was identified by the CONTAM Panel for skin cancer. This value is the BMDL from a model average of BMD modelling on skin cancer. 
...
The risk managers has informed the risk managers that they find a certainty of 90% or more as acceptable for decision making. 
...
Unfortunately, this result is not enough to reach a conclusion whether a High Exposure is below the Reference Point (i.e. that the MOE is greater than `r th`). 
...
To save time, and avoid asking the experts to make judgements on these probability distributions, one can specify the probability distribution from available information, combine uncertainties by performing the probability calculations, and then ask the experts to make a judgement on the outcome of interest directly. 
...
In my analysis, uncertainty in the ratio HE over RP (i.e. 1/MOE) offers a better visualisation of the generated distribution. 
...
For the purpose of this exercise we assume that the experts agreed to not adjust the %probability that the MOE is above 1. 


## Conclusion

In my analysis, the attempts to arrive at a high certainty that the MOE is acceptable (i.e. above 1) has failed. This is because the level of certainty for the outcome of interest is `r round(mean(rp/he>1)*100)` % which is not in the region of practical certainty (i.e. <10% or >90%). 
...
View the third tutorial video [Refined options for assessing uncertainty](https://www.youtube.com/watch?v=wqNBBOeH7WQ) (30 minutes).
